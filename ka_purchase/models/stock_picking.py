            ...
            if (reception_steps == 'two_steps' and backorder_location_usage == "supplier" and pick.location_id.usage == "transit") or \
               (reception_steps == 'one_step' and pick.location_id.usage == 'supplier'):
                ...
                for po in self.env['purchase.order'].browse(purchase_id):
                    ...
                    # when user receive products from supplier, system will create vendor bill (NTB) automatically. 
                    # set ka_number in vendor bill with format : PO number/kode NTB                         
                    inv_account_id = po.purchase_category_id.property_account_payable_id.id if po.order_type == 'lokal' else po.partner_id.property_account_payable_id.id
                    invoice_id = self.env['account.invoice'].with_context(type='in_invoice').create({
                                        ...
                                        'date_invoice' : self.date_transfer,
                        ...
                        for prod in move_products:
                            if prod.get('product_id') == line.product_id.id:
                                ...
                                if data.get('quantity') > 0:
                                    ...
                                    # compute denda
                                    scheduled_date = datetime.strptime(line.order_id.date_planned, '%Y-%m-%d %H:%M:%S')
                                    for move in pick.move_lines:
