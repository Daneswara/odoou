    ...
    def button_confirm(self):
        for order in self:
            ...
            for l in order.order_line:
                if not l.product_id:
                    if not self.purchase_category_id.is_direct_purchase:
    ...
    def onchange_date_planned(self):
        for order in self:
            ...
            for line in order.order_line:
                line.date_planned = order.date_planned
...
class PurchaseOrderLine(models.Model):
    ...
    def get_last_sp(self, prod_id, date_order, order_id):
        ...
        query = order_line.search([('product_id', '=', prod_id), ('tanggal_sp', '<', date_order), ('order_id', '!=', order_id)], order = 'tanggal_sp desc')
        filter_result = query.filtered(lambda x: len(x.order_id.invoice_ids) != 0)
        limit = 3
        res = []
