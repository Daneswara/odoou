...
class AccountInvoice(models.Model):
    ...
    def _prepare_invoice_line_from_po_line(self, line):
        ...
        account = invoice_line.with_context(analytic=line.account_analytic_id).get_invoice_line_account('in_invoice', line.product_id, self.purchase_id.fiscal_position_id, self.env.user.company_id)
        if account:
...
#                     src_penalty_obj = self.env['account.penalty'].search([('invoice_id','=',inv_create.id)])
    ...
    def action_force_draft(self):
        for this in self:
            #dibatasi hanya untuk NTB berstatus paid
            if this.state == 'paid':
                if this.intercompany_invoice_type == 'lokal':

                    raise UserError('Tidak dapa merubah status secara paksa! Bukti kas dan journal sudah terbentuk. Mohon hubungi Administrator.')
                elif this.intercompany_invoice_type == 'rkin':
                    ...
                    # Cari NTB direksi
                    invoice_direksi = self.env['account.invoice'].search([('source_invoice_id','=',this.id)])

                    if invoice_direksi:
                        if invoice_direksi.state == 'paid':
                            raise UserError('Tidak dapa merubah status secara paksa! Bukti kas dan journal direksi sudah terbentuk. Mohon hubungi Administrator.')
                        elif invoice_direksi.state == 'open':
                            # hilangkan move name kemudian cancel invoice supaya bisa jadi draft
                            # move_name = False -> cancel-> commit dulu supaya bisa dihapus -> delete
                            invoice_direksi.move_name = False
                            invoice_direksi.action_invoice_cancel()
                            self._cr.commit()
                            invoice_direksi.unlink()
                        else:
                            raise UserError('Eits tunggu dulu ada yg salah, sepertinya state NTB salah')

            else:
                raise UserError('Tidak dapa merubah status secara paksa! NTB tidak berstatus paid.')
