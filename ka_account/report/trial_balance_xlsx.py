...
class Trial_Balance_Xlsx(ReportXlsx):
    ...
    def generate_xlsx_report(self, workbook, data, account_report):
        ...
        row = 4
        for account in self.env['account.financial.report'].search([('parent_id','=',account_report.account_report_id.id)], order='sequence asc').filtered(lambda x: len(x.account_ids) != 0) :
            row +=1
            ...
            for acc in account.account_ids:
                ...
                self.env.cr.execute("""select sum(move_line.balance) from account_move_line move_line
                                ...
                                where move.state = 'posted' and move_line.account_id = %s and move_line.date < %s and move_line.date >= %s""", (acc.id, account_report.date_from, date_jan))
                ...
                self.env.cr.execute("""select sum(move_line.debit) from account_move_line move_line
                                ...
                                join account_journal journal on move_line.journal_id = journal.id
                                join account_move move on move_line.move_id = move.id
                            where move.state = 'posted' and move_line.account_id = %s and move_line.date >= %s and move_line.date <= %s""", (acc.id, account_report.date_from, account_report.date_to))
                ...
                # else:
                #     date_from1 = fields.Date.from_string(account_report.date_from).replace(day=1, month=1)
                #
                #     self.env.cr.execute("""select sum(move_line.balance) from account_move_line move_line
                #                                         join account_account account on move_line.account_id = account.id
                #                                         join account_journal journal on move_line.journal_id = journal.id
                ...
                #                         (acc.id, account_report.date_from, date_from1))
                #     open_balance = self.env.cr.dictfetchone()
                #     if open_balance['sum'] == None:
                #         open_balance['sum'] = 0.0
                ...
                #     if credit['sum'] == None:
                #         credit['sum'] = 0.0
                #     credit_str = str('{0:,.2f}'.format(float(credit['sum']))).replace('.', '%').replace(',',
                #                                                                                         '.').replace(
                #         '%', ',')
                #
                #     end_balance = open_balance['sum'] + debit['sum'] - credit['sum']
