...
class daily_cash_qweb(models.AbstractModel):
    ...
    def get_journal(self):
        ...
        journal_obj = journal_src.search([('type', 'in', ('bank','cash')),('active','=',True),('company_id','child_of',self.env.user.company_id.id)], order='type desc')
        return journal_obj    
    ...
    def get_opening_balance(self, journal_id, report_date):
        ...
        start_date_obj = datetime.strptime(report_date, DF).replace(month=1).replace(day=1)
        start_date = start_date_obj.strftime(DF)
        ...
        self._cr.execute("select sum(debit) from account_move_line move_line join account_move move on move_line.move_id = move.id where move.state = 'posted' and move_line.date >= %s and move_line.date < %s and move_line.account_id = %s",
                         (start_date, report_date, debit_account))
        debit = self._cr.fetchone()[0] or 0
        self._cr.execute("select sum(credit) from account_move_line move_line join account_move move on move_line.move_id = move.id where move.state = 'posted' and move_line.date >= %s and move_line.date < %s and move_line.account_id = %s",
                         (start_date, report_date, credit_account))
