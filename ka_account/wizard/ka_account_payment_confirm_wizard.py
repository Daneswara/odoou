        ...
        if active_ids:
            ...
            for payment in self.env['ka_account.payment'].browse(active_ids):
                ...
                if payment.invoice_id:
                    date_splited = payment.invoice_id.date_invoice.split('-')
                    vendor_invoice_date = '%s-%s-%s' % (date_splited[2],date_splited[1],date_splited[0])

                invoice_account = payment.account_id.id
                label_vals_one = (_('%s; %s') % (payment.description or '-', vendor_invoice_date or '-'))
                if payment.invoice_id:
                    invoice_account = payment.invoice_id.purchase_category_id.property_account_payable_id.id
                    label_vals_one = (_('NTB: %s; %s') % (payment.invoice_id.ka_number or '-', vendor_invoice_date or '-'))
                ...
                if payment.amount_pph > 0:
                    vals_pph = {
                        ...
                        'name' : (_('PPH: %s; %s') % (payment.no_kwitansi, tax_date)),
                ...
                if payment.amount_penalty > 0:
                    vals_penalty = {
                        ...
                        'name' : (_('Denda: %s; %s') % (payment.no_kwitansi, vendor_invoice_date)),
            ...
            return {
                ...
                'context': {'default_type':'outbound', 'invoice_type': 'in_invoice'},
                'target': 'current'
            }
