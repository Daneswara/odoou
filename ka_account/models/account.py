...
class AccountAccount(models.Model):
    ...
    report_line_ids = fields.Many2many('account.financial.report', 'account_account_financial_report', 'account_id',
                                       'report_line_id', 'Group Laporan')
...
class AccountJournal(models.Model):
    ...
    active = fields.Boolean(string="Active", default=True, store=True, help="If unchecked, it will allow you to hide the account journal without removing it.")
...
class AccountPayableCategory(models.Model):
    ...
    property_account_payable_id = fields.Many2one('account.account', company_dependent=True, required=True,
                                                  string="Perkiraan Hutang",
                                                  domain="[('internal_type', '=', 'payable'), ('deprecated', '=', False)]")
    kode_ntb = fields.Char("Kode NTB", company_dependent=True)
    is_direct_purchase = fields.Boolean("Pembelian Langsung",
                                        help="Pembeliaan yang tidak akan menimbulkan Hutang, jadi nantinya langsung dibiayakan pada saat pembauaran")
