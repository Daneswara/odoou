...
class AccountBankStatement(models.Model):
    ...
    def _compute_move(self):
        self.inbound_total = sum([line.amount for line in self.line_ids if line.journal_voucher_id.type=='inbound'])
        self.outbound_total = sum([line.amount for line in self.line_ids if line.journal_voucher_id.type=='outbound'])
        
    inbound_total = fields.Monetary('Masuk', compute='_compute_move', store=False, help='Total Inbound as calculated based on transaction lines')
    outbound_total = fields.Monetary('Keluar', compute='_compute_move', store=False, help='Total Outbound as calculated based on Opening Balance and transaction lines')
    ...
    def local_to_utc(self, source_datetime):
        user_tz = self.env.user.tz or pytz.utc
        local = pytz.timezone(user_tz)
        naive = datetime.strptime(source_datetime, '%Y-%m-%d')
    ...
    def action_pull_bukti_kas(self):
        #karena approve date adalah datetime, maka date perlu di konversi menjadu datetime(UTC)
        for this in self:
