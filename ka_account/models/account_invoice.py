...
class AccountInvoice(models.Model):
    ...
    def view_related_payments(self):
        ...
        '''
        action = self.env.ref('ka_account.action_open_payment')
        ...
        if len(self.ka_payment_ids) > 1:
            result['domain'] = "[('id', 'in', " + str(self.ka_payment_ids.ids) + ")]"
        else:
            res = self.env.ref('ka_account.view_ka_account_payment_form', False)
            result['views'] = [(res and res.id or False, 'form')]
            result['res_id'] = self.ka_payment_ids.id
        return result
